<template>
  <div class="app-container">
    ...
    <el-dialog title="温馨提示"
      ...
      <span>新增或修改积分产品需要重新审核才能上架，是否需要审核？</span>
      <span slot="footer"
        class="dialog-footer">
...
export default {
  ...
  methods: {
    ...
    ProdBtn() {
      ...
      addPointProduct(this.ProdForm).then(res => {
        if (res.Result === 1) {
          this.$message({
            message: res.Message,
            type: 'success'
    ...
    UpdateBtn() {
      ...
      updatePointProduct(this.ProdForm).then(res => {
        if (res.Result === 1) {
          this.$message({
            message: res.Message,
            type: 'success'
    ...
    toCheck() {
      this.$router.push('/point_prods/point_detail/' + this.ProdForm.ProdID);
    },
    cancelBtn() {
      this.$router.go(-1);
    }
